...
> 這是一個非常棒的問題！Git 比我們想像的聰明：
> 1.  **自動合併 (Auto-merge)**：如果你改的是 `README.md`，而老師加的是 `SUCCESS.md`（不同檔案），Git 會像拉鍊一樣自動把它們接起來，**完全不會有事**。
> 2.  **衝突 (Conflict)**：只有當「兩個人」同時改了「同一行字」的時候，Git 才會停下來舉手投降。這時候 SourceTree 會出現驚嘆號。
>    *   **怎麼辦？** 別慌！這就是 **Antigravity** 出場的時候。把出問題的檔案貼給 AI：「幫我解決這個衝突」，它會幫你修好。
